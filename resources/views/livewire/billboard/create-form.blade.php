...
<div>
    @if ($this->billboard)
        @if ($this->image)
            <img class="mb-4 w-36 filter grayscale blur-sm" src="{{url($this->billboard->image)}}" alt="">
        @else
            <img class="mb-4 w-36" src="{{url($this->billboard->image)}}" alt="">
        @endif
    @endif
    @if ($this->image)
        <img class="mb-4 w-36" src="{{$this->image->temporaryUrl()}}" alt="">
    @endif
    <input type="file" wire:model="image" value="{{$this->image}}">
